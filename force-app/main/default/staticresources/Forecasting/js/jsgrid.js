...
(function(window, $, undefined) {
    ...
    Grid.prototype = {
        ...
        sortAscClass: "jsgrid-header-sort opp-list-sort--asc-on",
        sortDescClass: "jsgrid-header-sort opp-list-sort--desc-on",
        ...
        _getItemFieldValueByDot: function(item, field){
            var props = field.name.split(field.nameSplitsFlag);
        ...
        _getItemFieldValueDirectly: function(item, field){
            return item[field.name];
        },

        _getItemFieldValue: function(item, field) {
            var fieldValue = field.nameSplits ? this._getItemFieldValueByDot(item, field) : this._getItemFieldValueDirectly(item, field);
            if(field.dependency)
            {
                fieldValue = {value: fieldValue, dependency: null};
                fieldValue.dependency = field.dependencyNameSplits ? this._getItemFieldValueByDot(item, { name: field.dependencyName, nameSplitsFlag: field.nameSplitsFlag}) : this._getItemFieldValueDirectly(item, { name:field.dependencyName});
            }
            return fieldValue;
        },

        _setItemFieldValueByDot: function(item, field, value){
            var props = field.name.split(field.nameSplitsFlag);
            var current = item;
            var prop = props[0];
        ...
        _setItemFieldValueDirectly: function(item, field, value){
        ...
        _setItemFieldValue: function(item, field, value) {
            if(field.nameSplits)
            {
                this._setItemFieldValueByDot(item, field, value);
            }else
            {
                this._setItemFieldValueDirectly(item, field, value);
            }
        },

        customSortFunc: function(field, order)
        {
          if(field)
          {
             var scope = angular.element($(oppListV2GlobalConfiger.AngularContainer)).scope();
             scope.$apply(function(){
              if(field.sortDependency && field.sortByDependency)
              {
                  scope.sortOppList(scope.oppListV2.convertFieldName(field.sortDependency, field.nameSplitsFlag));
              }else {
                  scope.sortOppList(scope.oppListV2.convertFieldName(field.name, field.nameSplitsFlag));
              }
            });
          }
        },
        ...
        sort: function(field, order) {
            ...
            if(this.customSort)
            {
                this.customSortFunc(this._sortField, this._sortOrder);
            }else {
           ...
           var sortFactor = this._sortFactor(),
            ...
            if(sortField) {
                this.data.sort(function(item1, item2) {
                    var v1 = jsGrid._getItemFieldValue(item1, sortField),
                        v2 = jsGrid._getItemFieldValue(item2, sortField);
                    if(sortField.dependency)
                    {
                        if(sortField.sortByDependency)
                        {
                           v1 = v1.dependency;
                           v2 = v2.dependency;
                        }else {
                    ...
                    return sortFactor * sortField.sortingFunc( v1, v2);
                });
            }
        },
...
(function(jsGrid, $, undefined) {
    ...
    var validators = {
        ...
        attrRangeLength: {
           message: "Entered length is out of the range.",
           validator: function(value, _, param) {
               return value[param[2]].length >= param[0] && value[param[2]].length <= param[1];
           }
        },
        ...
        sfdcDate: {
           message: "Please in put valied date, not blank",
           validator: function(value, _, param){
               try {
                 new Date(value);
                 return true;
...
(function(jsGrid, $, undefined) {
    ...
    Field.prototype = {
        ...
        dependency: false,
        dependencyName: "",
        dependencyNameSplits: true,
        sortDependency: '',
        sortByDependency: false,
        ...
        headerTemplate: function() {
          var headerTitle = (this.title === undefined || this.title === null) ? this.name : this.title;
          var templateDom = $("<div class='jsGrid-c-header'>")
                             .append($("<i>").prop("class","jsGrid-c-header--icon opp-list-sort--asc fa fa-long-arrow-down "))
                             .append($("<i>").prop("class","jsGrid-c-header--icon opp-list-sort--desc fa fa-long-arrow-up "))
                             .append($("<div>").prop("class","jsGrid-c-header--content").text(headerTitle));
          return templateDom;
        },
        ...
        _getOrApply: function(value, context) {
            if($.isFunction(value)) {
                return value.apply(context, $.makeArray(arguments).slice(1));
            }
            return value;
        }
    };
...
(function(jsGrid, $, undefined) {
    ...
    ControlField.prototype = new Field({
        ...
        itemTemplate: function(value, item) {
            ...
            if(this._getOrApply(this.editButton, {'value':value, 'item':item}) === true ) {
                $result = $result.add(this._createEditButton(item));
            }

            if(this._getOrApply(this.deleteButton, {'value':value, 'item':item}) === true) {
