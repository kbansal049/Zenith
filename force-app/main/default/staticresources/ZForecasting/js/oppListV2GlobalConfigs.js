...
oppListV2GlobalConfiger.Fields.sfdcCheckbox = function(config) {
    jsGrid.Field.call(this, config);
};

//Fields definitions
oppListV2GlobalConfiger.Fields.sfdcCheckbox.prototype = new jsGrid.Field({
    align: 'center',
    itemTemplate: function(value) {
        return $("<input>").prop("type", "checkbox").prop("disabled", "disabled").prop("checked", value).val(value);
    },
    insertTemplate: function(value) {
        return this._insertPicker = $("<input>").prop("type", "checkbox").prop("checked", value).val(value);
    },
...
oppListV2GlobalConfiger.Fields.sfdcPicklist.prototype = new jsGrid.Field({
    ...
    itemTemplate: function(value, item) {
        ...
        if (this.name == 'MyForecastCategory'
           && item.MyForecastCategory != item.LevelMinusOneForecastCategory
           && this.scope.MyUserLevel != 'Rep') {
            content = $('<span>').css("color", "red").text((result === undefined || result === null) ? "" : this.scope.unescape(result));
        } else {
            content = $('<span>').text((result === undefined || result === null) ? "" : this.scope.unescape(result));
        }
        elem.append(content);
    ...
    editTemplate: function(value, item) {
        ...
        if (item.OwnedByViewingUser === false || item.RecordEditable === false || this.editing === false)
