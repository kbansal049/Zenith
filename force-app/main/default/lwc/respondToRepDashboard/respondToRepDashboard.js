...
import getTotalResponedToRepData from '@salesforce/apex/DashBoardComponent.getTotalResponedToRepData';
import updateOpportunity from '@salesforce/apex/DashBoardComponent.updateOpportunity';
import getNextResponedToRepData from '@salesforce/apex/DashBoardComponent.getNextResponedToRepData';
...
export default class Responed_To_Rep_Dashboard extends LightningElement {
    ...
    renderedCallback(){
        if(this.isBackButtonClicked && this.currentPage){
 /*           this.template.querySelector('lightning-datatable').scrollIntoView({
    ...
    connectedCallback() {
        loadStyle(this, cssStaticResource).then(()=>{
            ...
            getDataForExporting({ statusVal: 'PO Pending with Sales' })
            .then(result => {
            ...
            if((this.isBackButtonClicked && this.currentPage) || this.currentPage){
                this.start = this.currentPage.start;
                this.end = this.currentPage.end;
                this.selectedPageOption = this.currentPage.selectedPageOption;
                this.pageNumber = this.currentPage.pageNumber
                this.sortedBy = this.currentPage.sortBy;
                this.sortDirection = this.currentPage.sortDirection;
                this.totalPages = this.currentPage.totalPages;
                this.totalRecords = this.currentPage.totalRecords;
                this.totalSumArr = this.currentPage.totalSumArr;
                this.getResponedToRepDataMethod();
    ...
    onRowActionClick(event) {
        ...
        if(actionName === 'Reason'){
            ...
            let currentPage = {};
            currentPage.start = this.start;
            currentPage.end = this.end;
            currentPage.selectedPageOption = this.selectedPageOption;
            currentPage.sortedBy = this.sortedBy;
            currentPage.sortDirection = this.sortDirection;
            currentPage.pageNumber = this.pageNumber;
            currentPage.totalPages = this.totalPages;
            currentPage.totalRecords = this.totalRecords;
            currentPage.totalSumArr = this.totalSumArr;
            currentPage.Id = event.detail.row.Id;           
            var filteredArray  = [...this.finalDataTable].filter((data)=>{
                if(data.Id === event.detail.row.Id){
                    return  data.Id === event.detail.row.Id;
                }                
        });
        currentPage.Ticket = (filteredArray[0].Ticket__c).replace('-','%2D');
            const selectedEvent = new CustomEvent("showparentcomponent", {
                detail: { recordId: this.orderTrackerId, showParent: this.isShowParentOrderTrackerComponent, currentPage: currentPage }
            });
