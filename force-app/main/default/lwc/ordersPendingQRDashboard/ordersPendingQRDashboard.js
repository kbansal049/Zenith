...
import getTotalOrdersPendingQR from '@salesforce/apex/DashBoardComponent.getTotalOrdersPendingQR';
import getNextOrdersPendingQRData from '@salesforce/apex/DashBoardComponent.getNextOrdersPendingQRData';
...
/*    {
    ...
    { label: 'Last Status Change', sortable: true,fieldName: 'Live_Status_Change_Time_Difference__c',type: 'text', cellAttributes: { alignment: 'left' },}, 
];
export default class Orders_Pending_QR_dashboard extends LightningElement {
    @track orderTrackerId;
    @track isShowParentOrderTrackerComponent = true;
    @track mainOrdersPendingQR;
    @track tableOrdersPendingQR;
    ...
    onHandleSort(event) {
        ...
        this.getOrdersPendingQRDataMethod();
        const cloneData = [...this.tableOrdersPendingQR];
    ...
    renderedCallback(){
        ...
        if(this.isBackButtonClicked && this.currentPageqr){
    ...
    connectedCallback() {
        ...
        getDataForExporting({ statusVal: 'PO Pending for QR' })
            .then(result => {
            ...
            if(this.isBackButtonClicked && this.currentPageqr){
                ...
                this.start = this.currentPageqr.start;
                this.end = this.currentPageqr.end;
                this.selectedPageOption = this.currentPageqr.selectedPageOption;
                this.pageNumber = this.currentPageqr.pageNumber
                this.sortedBy = this.currentPageqr.sortBy;
                this.sortDirection = this.currentPageqr.sortDirection;
                this.totalPages = this.currentPageqr.totalPages;
                this.totalRecords = this.currentPageqr.totalRecords;
                this.totalSumArr = this.currentPageqr.totalSumArr;
                this.getOrdersPendingQRDataMethod();
                this.handleSpinnerLoadingOff();
            }
            else{
                getTotalOrdersPendingQR()
                .then(result => {
                    ...
                    this.mainOrdersPendingQR = this.createNewData(respondedToRepList['OrdersPendingQRList'], false);
                    this.start = 1;
                    ...
                    //     this.mainOrdersPendingQR[i].New_Upsell_ACV__c = (this.mainOrdersPendingQR[i].New_Upsell_ACV__c == 0) ? '' : this.mainOrdersPendingQR[i].New_Upsell_ACV__c;
                    //     this.mainOrdersPendingQR[i].Renewal_ACV__c = (this.mainOrdersPendingQR[i].Renewal_ACV__c == 0) ? '' : this.mainOrdersPendingQR[i].Renewal_ACV__c;
                    //     this.mainOrdersPendingQR[i].ACV__c = (this.mainOrdersPendingQR[i].ACV__c == 0) ? '' : this.mainOrdersPendingQR[i].ACV__c;
                    //     this.mainOrdersPendingQR[i].TCV__c = (this.mainOrdersPendingQR[i].TCV__c == 0) ? '' : this.mainOrdersPendingQR[i].TCV__c;
                    //     tempList.push(this.mainOrdersPendingQR[i]);
                    // }
                    if (this.totalRecords <= this.selectedPageOption) {
                    ...
                    this.tableOrdersPendingQR = this.mainOrdersPendingQR;
    ...
    getOrdersPendingQRDataMethod() {
        getNextOrdersPendingQRData({
            ...
            .then(result => {
                ...
                // }
                this.tableOrdersPendingQR = orders;
                this.tableOrdersPendingQR= this.createNewData(this.tableOrdersPendingQR, false);
                this.mainTableFunction();
                this.totalPages = Math.ceil(this.totalRecords / this.selectedPageOption);
    ...
    onRowActionClick(event) {
        ...
        if(actionName === 'Reason'){
            ...
            let currentPageqr = {};
            currentPageqr.start = this.start;
            currentPageqr.end = this.end;
            currentPageqr.selectedPageOption = this.selectedPageOption;
            currentPageqr.sortedBy = this.sortedBy;
            currentPageqr.sortDirection = this.sortDirection;
            currentPageqr.pageNumber = this.pageNumber;
            currentPageqr.totalPages = this.totalPages;
            currentPageqr.totalRecords = this.totalRecords;
            currentPageqr.totalSumArr = this.totalSumArr;
            currentPageqr.Id = event.detail.row.Id;           
            var filteredArray  = [...this.finalDataTable].filter((data)=>{
                if(data.Id === event.detail.row.Id){
                    return  data.Id === event.detail.row.Id;
                }                
        });
        currentPageqr.Ticket = (filteredArray[0].Ticket__c).replace('-','%2D');
            const selectedEvent = new CustomEvent("showparentcomponent", {
                detail: { recordId: this.orderTrackerId, showParent: this.isShowParentOrderTrackerComponent, currentPageqr: currentPageqr }
    ...
    //     const selectedEvent = new CustomEvent("showparentcomponent", {
    //         detail: {recordId:this.orderTrackerId,showParent:this.isShowParentOrderTrackerComponent}
    //     });
    ...
    mainTableFunction() {
        var objTotal = {}
        this.finalDataTable = this.tableOrdersPendingQR;
        objTotal.Id = 'objtotalid123'
        objTotal.End_User__c = 'Total';
        objTotal.Proposal_for_initiating_QC__c = ' ';
        objTotal.SO__c = ' ';
